...
def FetchEQU( job     =None , \
    ...
    # --- [4] ダウンサンプリング -  #
    if ( config["equ_DownConvert"] ):
        xd = np.linspace( xAxis[0], xAxis[-1], config["equ_LId"] )
        yd = np.linspace( yAxis[0], yAxis[-1], config["equ_LJd"] )
        dData = np.zeros( (12,config["equ_LJd"],config["equ_LId"] ) )
        for i in range(12):
            dData[i] = r2r.rct2rct( xp=xAxis, yp=yAxis, data=Data[i], xg=xd, yg=yd )
        Data = dData
        flux = r2r.rct2rct( xp=xAxis, yp=yAxis, data=flux, xg=xd, yg=yd )
        xAxis, yAxis = xd, yd
    # --- [5] 切り出し : xRange, yRange で 検索して 返却 -- #
