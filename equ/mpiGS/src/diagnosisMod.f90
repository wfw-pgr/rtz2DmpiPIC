...
contains
  ...
  ! === measure__perimeter                             === !
  ! ====================================================== !
  function measure__perimeter( map, N1, N2, dx1, dx2 )
    implicit none
    integer         , intent(in) :: N1, N2
    double precision, intent(in) :: map(N1,N2), dx1, dx2
    double precision             :: measure__perimeter
       ...
       do j=jBtm, N2-jTop
          if ( map(i,j)*map(i,j-1).le.0.d0 ) then
             ...
             if ( Nisec(i).le.2 ) then
                Xisec(X_,Nisec(i),i) = dx1*dble(i-1)
                Xisec(Y_,Nisec(i),i) = dx2*dble(j-1)  &
    ...
    enddo
    !  -- [2-2] Left & Right  -- !
    lLeft  = ( Xisec(Y_,Upr,iEdge(1)) - Xisec(Y_,Lwr,iEdge(1)) )
    lRight = ( Xisec(Y_,Upr,iEdge(2)) - Xisec(Y_,Lwr,iEdge(2)) )
    !  -- [2-3] Top  & Bottom -- !
    Xnext(X_,Lwr) = Xisec(X_,Lwr,iEdge(1))
    Xnext(Y_,Lwr) = Xisec(Y_,Lwr,iEdge(1))
    Xnext(X_,Upr) = Xisec(X_,Upr,iEdge(1))
    Xnext(Y_,Upr) = Xisec(Y_,Upr,iEdge(1))
    lBot = 0.d0
    lTop = 0.d0
    do i=iEdge(1)+1, iEdge(2)
       if ( Nisec(i).eq.2 ) then
          lBot = lBot + sqrt( ( Xisec(X_,Lwr,i) - Xnext(X_,Lwr) )**2 &
               &            + ( Xisec(Y_,Lwr,i) - Xnext(Y_,Lwr) )**2 )
          lTop = lTop + sqrt( ( Xisec(X_,Upr,i) - Xnext(X_,Upr) )**2 &
               &            + ( Xisec(Y_,Upr,i) - Xnext(Y_,Upr) )**2 )
          Xnext(X_,Lwr) = Xisec(X_,Lwr,i)
          Xnext(Y_,Lwr) = Xisec(Y_,Lwr,i)
          Xnext(X_,Upr) = Xisec(X_,Upr,i)
          Xnext(Y_,Upr) = Xisec(Y_,Upr,i)
       endif
    enddo
    ...
    ! --- [3] return  perimeter                          --- !
    ! ------------------------------------------------------ !
    measure__perimeter = lLeft + lRight + lBot + lTop

    return
  end function measure__perimeter
  ...
  !   use constants    , only : LI, LJ, limposr, limposz, limidx, valfe, jobdir
  ...
  !   implicit none
  !   integer                :: i,j
  !   double precision       :: fx(LJ,LI) , psis(LI,LJ)
  ...
  !   fx(:,:)  = 1.d0
  !   area     = pol_integral( z, r, fx , LI, LJ, psi, psilim )
  !   Ppol     = pol_integral( z, r, prs, LI, LJ, psi, psilim )
  !   Ppol     = Ppol / area
  ...
  !      enddo
  !   enddo
  !   pLength  = Perimeter( psis, LI, LJ, dz, dr )
  !   Bpol2    = ( 0.5d0 * ( Itotal / pLength )**2 )
  !   Betap2   = valfe**2 * Ppol / Bpol2
    
  !   !  --- [3] Phi total ( Toroidal Flux ) --- !
  !   Phit     = pol_integral( z, r, Bt , LI, LJ, psi, psilim )
  !   Itot     = pol_integral( z, r, Jt , LI, LJ, psi, psilim )
  ...
  ! function pol_integral( z, r, var, LI, LJ, psi, psilim )

  !   use NewtonRaphson, only       : cSplineNewtonRaphson1D
  !   implicit none
  !   integer         , intent(in) :: LJ, LI
  !   double precision, intent(in) :: r(LJ), z(LI), var(LI,LJ)
  !   double precision, intent(in) :: psi(LI,LJ), psilim
  !   integer                      :: i, j, inflag, flag, edger, edgel
  !   double precision             :: start, end, dfx
  !   double precision             :: rintegrated(LI-1), fx(LJ)
  ...
  !   use constants    , only : LJ, LI, mr, vthcv, TiTe, jobdir
  ...
  !           &        / sqrt( Br(LI/2,j)**2 + Bt(LI/2,j)**2 + Bz(LI/2,j)**2 )
  !   enddo ! -- Larmor Radius    -- !
  ...
  !   use constants,          only : LJ, LI, jobdir
