...
def vdfRegion( vcnFile=None, config=None, LI=None, LJ=None, x1Min=None, x1Max=None, x2Min=None, x2Max=None ):
    ...
    if ( LI      is None ): LI      = config["pic_LId"]
    if ( LJ      is None ): LJ      = config["pic_LJd"]
    ...
    print( Ndiv, Nset, vdfBox )

    cfs.configSettings( config=config, configType="cMap2D_def" )
    cMap = np.zeros( (LJ,LI) )
    fig  = clm.cMap2D( xAxis=x2Axis, yAxis=x1Axis, cMap=cMap, config=config, NoInstant=True )
    for i in range( Nset ):
