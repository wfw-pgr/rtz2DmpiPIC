...
contains
  ...
  subroutine Flux_and_Current
    ...
    implicit none
    include 'mpif.h'
    integer                     :: i, j, ptOXO_(3,2), ierr
    integer                     :: isOpt=0, iFixed=0
    double precision            :: dS, time , psiNX_   , mergR_
    ...
    if ( myRank.eq.0 ) then
       ...
       ! -- Update or No-Update -- !
       if ( ( abs( mergR_-mergR ).gt.0.1d0 ).or.( mergR_.lt.0.d0 ) ) then
          ptOXO = ptOXO_
       endif
    endif
    ...
    if ( Flag__FixedXpt ) then
       ...
       call ReconnectionRate( ptOXO, RateR, mergR, Flag__sOpt, Flag__FixedXpt )
    endif
    ! -------------------------- !
    ! --- [3] Plasma Current --- !
    ! -------------------------- !
