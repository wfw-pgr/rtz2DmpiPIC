...
contains
  ...
  subroutine WriteField( onGrid )
    use RgridMod , only            : LIr, LJr, BgR, fgR, JgR, ugR
    use BgridMod , only            : LIg, LJg, BgB, fgB, JgB, ugB
    ...
    write(50) Bwr
    close(50)
    write(6,'(a25,1x,a)') trim( FileName ), '   has been outputted...'
    !   -- [2-2] frp.bin    -- !
  ...
  subroutine WriteParam( onGrid )
    use RgridMod , only       : LIr, LJr
    use Field    , only       : LIg, LJg
    use constants, only       : wpewce, vthcv, valfe, TiTe, rhofloor, PEpic
    use constants, only       : job, dz_Debye, dr_Debye, zMin, zMax, rMin, rMax
    use variables, only       : dz, dr
    implicit none
    integer                  :: LIh, LJh
    ...
    write(30,'(2(a12,2x),e15.8)') 'dz'      ,'double', dz
    write(30,'(2(a12,2x),e15.8)') 'dr'      ,'double', dr
    write(30,'(2(a12,2x),e15.8)') 'dz_Debye','double', dz_Debye
    write(30,'(2(a12,2x),e15.8)') 'dr_Debye','double', dr_Debye
    ...
    write(30,'(2(a12,2x),e15.8)') 'x1Min'   ,'double', zMin
    write(30,'(2(a12,2x),e15.8)') 'x1Max'   ,'double', zMax
    write(30,'(2(a12,2x),e15.8)') 'x2Min'   ,'double', rMin
    write(30,'(2(a12,2x),e15.8)') 'x2Max'   ,'double', rMax
    write(30,'(2(a12,2x),i15)'  ) 'PEtot'   ,'long'  , PEpic
    close(30)
    write(6,'(4x,a,a)') trim(FileName), ' has been outputted...'
    write(6,*)
  ...
  subroutine WriteF_MPI( onGrid )
    ...
    double precision   :: divB, sum_divB, sum_divB2
