...
import fLIB.fLIB__RefppDecompose     as rfD
...
def EdJAnalysis( job    =None, jobDir=None, ksteps =None , CnsFile=None, \
                 ...
                 NewFile=True, Flag__CallCalculater=False , Flag__CallPlotter=True, MaskType=None, Flag__integral=True ):
    # ------------------------------- #
    # --- [1]   引数チェック      --- #
    # ------------------------------- #
...
# ===    J.Eを計算して表示             === #
# ======================================== #
def EdJ_calculator( job  =None, jobDir=None, kstep =None, config=None, MaskType=None ):
    # ------------------------ #
    # --- [1] 引数チェック --- #
    # ------------------------ #
    if ( config  is None ): config = lcf.LoadConfig()
    ...
    Data  = fpc.FetchPIC( job=job , jobDir=jobDir, kstep =kstep, keys=keys, \
                          FilterKeys=keys, RawData=True, config=config )
    xAxis = Data["xAxis"]
    yAxis = Data["yAxis"]
    Flux  = Data["psi"  ]
    #  -- [2-2] 準備         --  #
    ...
    ppDaxis  = ppD.MFppDecompose( r1=Data["Bx"], r2=Data["By"], r3=Data["Bz"] )
    # ---------------------------- #
    # --- [3]  E.Je ( elec. )  --- #
    # ---------------------------- #
    #  -- [3-1] Je// .E        --  #
    JeprlE   = ( Data["Jex"]*ppDaxis["vparax"]*Data["Ex"] + \
                 Data["Jey"]*ppDaxis["vparay"]*Data["Ey"] + \
                 Data["Jez"]*ppDaxis["vparaz"]*Data["Ez"]   )
    sJeprlE  = np.sum( JeprlE * volm * mask )
    #  -- [3-2] Je|1 .E        --  #
    Jepp1E   = ( Data["Jex"]*ppDaxis["vprp1x"]*Data["Ex"] + \
                 Data["Jey"]*ppDaxis["vprp1y"]*Data["Ey"] + \
                 Data["Jez"]*ppDaxis["vprp1z"]*Data["Ez"]   )
    sJepp1E  = np.sum( Jepp1E * volm * mask )
    #  -- [3-3] Je|2 .E        --  #
    Jepp2E   = ( Data["Jex"]*ppDaxis["vprp2x"]*Data["Ex"] + \
                 Data["Jey"]*ppDaxis["vprp2y"]*Data["Ey"] + \
                 Data["Jez"]*ppDaxis["vprp2z"]*Data["Ez"]   )
    ...
    # ---------------------------- #
    #  -- [4-1] Ji// .E        --  #
    JiprlE   = ( Data["Jix"]*ppDaxis["vparax"]*Data["Ex"] + \
                 Data["Jiy"]*ppDaxis["vparay"]*Data["Ey"] + \
                 Data["Jiz"]*ppDaxis["vparaz"]*Data["Ez"]   )
    sJiprlE  = np.sum( JiprlE * volm * mask )
    #  -- [4-2] Ji|1 .E        --  #
    Jipp1E   = ( Data["Jix"]*ppDaxis["vprp1x"]*Data["Ex"] + \
                 Data["Jiy"]*ppDaxis["vprp1y"]*Data["Ey"] + \
                 Data["Jiz"]*ppDaxis["vprp1z"]*Data["Ez"]   )
    sJipp1E  = np.sum( Jipp1E * volm * mask )
    #  -- [4-3] Ji|2 .E        --  #
    Jipp2E   = ( Data["Jix"]*ppDaxis["vprp2x"]*Data["Ex"] + \
                 Data["Jiy"]*ppDaxis["vprp2y"]*Data["Ey"] + \
                 Data["Jiz"]*ppDaxis["vprp2z"]*Data["Ez"]   )
...
if ( __name__=="__main__" ):
    ...
    EdJAnalysis( job   =args["job"]   , jobDir=args["jobDir"], \
                 ksteps=args["ksteps"], MaskType=MaskType )
